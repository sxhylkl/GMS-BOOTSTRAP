...
class Auth
{
    ...
    protected $_config = array(
        ...
        'AUTH_GROUP' => 'role',         // 角色数据表名
        'AUTH_RULE' => 'role_rule',    // 权限规则表
     ...
     *     ...)
     */
    public function getRules($uid, $type = 'rule')
    {
        ...
        if ('rule' == $type) {
            $user_groups = M()
                ...
                ->where($map)
                ->field('id,title,auth_rules')->select();
        } else {
            $user_groups = M()
                ->table($this->_config['AUTH_GROUP'])
                ->where($map)
                ->field('id,title,menu_rules')->select();
        }
